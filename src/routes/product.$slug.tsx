...
export const Route = createFileRoute("/product/$slug")({
  head: () => ({ meta: [{ title: "Ryvora Product" }, { name: "description", content: "Ryvora limited-run menswear." }, { property: "og:type", content: "product" }] }),
...
function ProductPage() {
  ...
  return <div className="min-h-screen bg-background"><SiteHeader/><div className="mx-auto max-w-7xl px-5 pt-28 sm:px-8"><Link to="/shop" className="eyebrow inline-flex items-center gap-2 text-muted-foreground hover:text-gold"><ArrowLeft className="h-3.5 w-3.5"/> Back to collection</Link></div>
    <section className="mx-auto grid max-w-7xl grid-cols-1 gap-12 px-5 py-10 sm:px-8 lg:grid-cols-2 lg:gap-20"><motion.div initial={{opacity:0,scale:1.04}} animate={{opacity:1,scale:1}} transition={{duration:1}} className="overflow-hidden bg-surface"><img src={productImage(product.image_key)} alt={product.title} width={1024} height={1280} className="aspect-4/5 w-full object-cover"/></motion.div>
      ...
      <ul className="mt-10 border-t border-border pt-6">{["500 GSM loopback fleece", "Garment dyed, pre-shrunk", "Made in limited runs"].map(spec => <li key={spec} className="flex items-center gap-3 border-b border-border py-3 text-sm text-muted-foreground"><span className="h-1 w-1 rounded-full bg-gold"/>{spec}</li>)}</ul></Reveal></div></section>
