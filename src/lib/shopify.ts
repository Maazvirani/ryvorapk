...
export async function fetchShopifyProductByHandle(handle: string): Promise<ShopifyProduct | undefined> {
  const products = await fetchShopifyProducts(1, `handle:${handle}`);
  return products[0];
}
...
export function getPrimaryImage(product: ShopifyProduct) {
  return product.node.images.edges[0]?.node;
}

export function getFirstAvailableVariant(product: ShopifyProduct) {
  return product.node.variants.edges.find((v) => v.node.availableForSale)?.node || product.node.variants.edges[0]?.node;
}

export function getOptionValues(product: ShopifyProduct, optionName: string) {
  return product.node.options.find((o) => o.name === optionName)?.values || [];
}
...
function isCartNotFoundError(userErrors: Array<{ field: string[] | null; message: string }>): boolean {
  return userErrors.some(
    (e) =>
      e.message.toLowerCase().includes("cart not found") || e.message.toLowerCase().includes("does not exist"),
  );
}
